...
class TestPerformance:
    ...
    def test_file_processing_time(self, sample_audio_data):
        ...
        assert processing_time < 1.0, f"File processing took {processing_time:.2f}s, expected < 1.0s"
    ...
    def test_concurrent_processing(self, mock_openai_client):
        ...
        assert total_time < 1.0, f"Concurrent processing took {total_time:.2f}s, expected < 1.0s"
...
class TestScalability:
    ...
    def test_large_file_handling(self):
        ...
        assert expected_chunks == 4, f"Expected 4 chunks for {large_file_size}B file, got {expected_chunks}"
    ...
    def test_multiple_users_simulation(self, mock_openai_client):
        ...
        def simulate_user(user_id):
            try:
                ...
                # Mock transcription
                mock_openai_client.audio.transcriptions.create.return_value.text = f"Transcription for user {user_id}"
                
                # Mock summarization
                mock_openai_client.chat.completions.create.return_value.choices[0].message.content = f"Summary for user {user_id}"
        ...
        # Sprawdzenie wyników
        assert results.qsize() == 10, f"Expected 10 successful operations, got {results.qsize()}"
...
class TestCachePerformance:
    ...
    def test_cache_hit_performance(self):
        ...
        assert hit_time < miss_time / 10, f"Cache hit ({hit_time:.4f}s) should be much faster than miss ({miss_time:.4f}s)"
